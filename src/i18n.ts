import i18n from "i18next";
import { initReactI18next } from "react-i18next";

i18n.use(initReactI18next).init({
  resources: {
    en: {
      translation: {
        "common.months": "Jan_Feb_Mar_Apr_May_Jun_Jul_Aug_Sep_Oct_Nov_Dec",

        "nav.appName": "Junior Tracker",
        "nav.chart": "Chart",
        "nav.addEvent": "Add event",
        "nav.stats": "Stats",
        "nav.settings": "Settings",
        "nav.logout": "Logout",
        "nav.login": "Login",
        "nav.register": "Register",

        "login.title": "Login",
        "login.email": "Email",
        "login.password": "Password",
        "login.submit": "Login",
        "login.failed": "Login failed",
        "login.noAccount": "No account?",
        "login.register": "Register",

        "register.title": "Register",
        "register.email": "Email",
        "register.password": "Password",
        "register.submit": "Register",
        "register.failed": "Registration failed",
        "register.hasAccount": "Already have an account?",
        "register.login": "Login",

        "addEvent.description": "Description",
        "addEvent.optional": "Optional",
        "addEvent.volume": "Volume (ml)",
        "addEvent.cancel": "Cancel",
        "addEvent.save": "Save",
        "addEvent.saving": "Saving...",
        "addEvent.saveFailed": "Failed to save event",
        "addEvent.rangeModeOpen": "Record the start",
        "addEvent.rangeModeDuration": "Duration (minutes)",
        "addEvent.willCreate": "Will create",
        "addEvent.changeType": "Change",

        "chart.bedtime": "Bedtime:",
        "chart.currentSleep": "Current sleep:",
        "chart.currentAwake": "Current awake:",
        "chart.fellAsleep": "Fell asleep:",
        "chart.wokeUp": "Woke up:",
        "chart.awake": "Awake:",
        "chart.wakeUp": "Wake up:",
        "chart.totalSleep": "Total sleep:",
        "chart.nightSleep": "Night sleep:",
        "chart.daySleep": "Day sleep:",
        "chart.cycle": "Cycle:",
        "chart.today": "Today",
        "chart.yesterday": "Yesterday",
        "chart.dayBefore": "Day before",
        "chart.child": "Child",
        "chart.dateFrom": "Date from",
        "chart.dateTo": "Date to",
        "chart.load": "Load",
        "chart.loadFailed": "Failed to load chart data",
        "chart.in": "in",
        "chart.noData": "No data",

        "et.sleep_start": "Sleep",
        "et.sleep_end": "Sleep end",
        "et.breastfeeding_start": "BF",
        "et.breastfeeding_end": "BF end",
        "et.formula": "Bottle",
        "et.food": "Food",
        "et.poo": "Poo",
        "et.bath": "Bath",

        "stats.title": "Statistics",
        "stats.cycleLength": "Cycle (sleep + awake)",
        "stats.daySleep": "Day sleep",
        "stats.nightSleep": "Night sleep",
        "stats.totalAwake": "Awake",
        "stats.formulaVolume": "Formula volume (ml)",
        "stats.formulaCount": "Formula feedings",

        "settings.loading": "Loading...",
        "settings.title": "Settings",
        "settings.childInfo": "Child info",
        "settings.name": "Name",
        "settings.timezone": "Timezone",
        "settings.save": "Save",
        "settings.saving": "Saving...",
        "settings.eventTypes": "Event types",
        "settings.colFormat": "Format",
        "settings.colColor": "Color",
        "settings.colKeywords": "Keywords",
        "settings.colLastEvents": "Last events",
        "settings.addEventType": "Add event type",
        "settings.nameRequired": "Name *",
        "settings.formatRequired": "Format *",
        "settings.colorPlaceholder": "rrggbb",
        "settings.keywordsPlaceholder": "comma separated",
        "settings.adding": "Adding...",
        "settings.add": "Add",
        "settings.rowSaving": "...",
        "settings.errorStatus": "Error {{status}}",
        "settings.networkError": "Network error",

        "register.childMode": "",
        "register.createChild": "Create child",
        "register.joinChild": "Join child",
        "register.childName": "Child name",
        "register.timezone": "Timezone",
        "register.birthdate": "Birthdate",
        "register.joinCode": "Join code",
        "register.joiningWithCode": "Registering to connect to child (code: {{code}})",

        "nav.invite": "Invite",
        "invite.title": "Invite a caregiver",
        "invite.hint": "Send this link to the person you want to invite. They register through it and get access to the child.",
        "invite.create": "Create invite link",
        "invite.creating": "Creating...",
        "invite.linkLabel": "Registration link",
        "invite.copy": "Copy",
        "invite.copied": "Copied",
        "invite.code": "Code",
        "invite.expiresAt": "Valid until: {{when}}",
        "invite.errorStatus": "Error {{status}}",
        "invite.networkError": "Network error",
        "invite.noChildren": "No children yet",
      },
    },
    ru: {
      translation: {
        "common.months": "Янв_Фев_Мар_Апр_Май_Июн_Июл_Авг_Сен_Окт_Ноя_Дек",

        "nav.appName": "Junior Tracker",
        "nav.chart": "График",
        "nav.addEvent": "Добавить событие",
        "nav.stats": "Статистика",
        "nav.settings": "Настройки",
        "nav.logout": "Выйти",
        "nav.login": "Войти",
        "nav.register": "Регистрация",

        "login.title": "Вход",
        "login.email": "Эл. почта",
        "login.password": "Пароль",
        "login.submit": "Войти",
        "login.failed": "Ошибка входа",
        "login.noAccount": "Нет аккаунта?",
        "login.register": "Зарегистрироваться",

        "register.title": "Регистрация",
        "register.email": "Эл. почта",
        "register.password": "Пароль",
        "register.submit": "Зарегистрироваться",
        "register.failed": "Ошибка регистрации",
        "register.hasAccount": "Уже есть аккаунт?",
        "register.login": "Войти",

        "addEvent.description": "Описание",
        "addEvent.optional": "Необязательно",
        "addEvent.volume": "Объём (мл)",
        "addEvent.cancel": "Отмена",
        "addEvent.save": "Сохранить",
        "addEvent.saving": "Сохранение...",
        "addEvent.saveFailed": "Ошибка сохранения",
        "addEvent.rangeModeOpen": "Записать начало",
        "addEvent.rangeModeDuration": "Длительность (минуты)",
        "addEvent.willCreate": "Будет создано",
        "addEvent.changeType": "Изменить",

        "chart.bedtime": "Отход ко сну:",
        "chart.currentSleep": "Текущий сон:",
        "chart.currentAwake": "Бодрствование:",
        "chart.fellAsleep": "Уснул:",
        "chart.wokeUp": "Проснулся:",
        "chart.awake": "Бодрствование:",
        "chart.wakeUp": "Пробуждение:",
        "chart.totalSleep": "Всего сна:",
        "chart.nightSleep": "Ночной сон:",
        "chart.daySleep": "Дневной сон:",
        "chart.cycle": "Цикл:",
        "chart.today": "Сегодня",
        "chart.yesterday": "Вчера",
        "chart.dayBefore": "Позавчера",
        "chart.child": "Ребёнок",
        "chart.dateFrom": "Дата с",
        "chart.dateTo": "Дата по",
        "chart.load": "Загрузить",
        "chart.loadFailed": "Ошибка загрузки",
        "chart.in": "через",
        "chart.noData": "Нет данных",

        "et.sleep_start": "Сон",
        "et.sleep_end": "Проснулся",
        "et.breastfeeding_start": "ГВ",
        "et.breastfeeding_end": "Конец ГВ",
        "et.formula": "Смесь",
        "et.food": "Еда",
        "et.poo": "Покакал",
        "et.bath": "Ванна",

        "stats.title": "Статистика",
        "stats.cycleLength": "Цикл (сон + бодрствование)",
        "stats.daySleep": "Дневной сон",
        "stats.nightSleep": "Ночной сон",
        "stats.totalAwake": "Бодрствование",
        "stats.formulaVolume": "Объём смеси (мл)",
        "stats.formulaCount": "Кормлений смесью",

        "settings.loading": "Загрузка...",
        "settings.title": "Настройки",
        "settings.childInfo": "Данные ребёнка",
        "settings.name": "Имя",
        "settings.timezone": "Часовой пояс",
        "settings.save": "Сохранить",
        "settings.saving": "Сохранение...",
        "settings.eventTypes": "Типы событий",
        "settings.colFormat": "Формат",
        "settings.colColor": "Цвет",
        "settings.colKeywords": "Ключевые слова",
        "settings.colLastEvents": "В последних",
        "settings.addEventType": "Добавить тип",
        "settings.nameRequired": "Название *",
        "settings.formatRequired": "Формат *",
        "settings.colorPlaceholder": "rrggbb",
        "settings.keywordsPlaceholder": "через запятую",
        "settings.adding": "Добавление...",
        "settings.add": "Добавить",
        "settings.rowSaving": "...",
        "settings.errorStatus": "Ошибка {{status}}",
        "settings.networkError": "Ошибка сети",

        "register.childMode": "",
        "register.createChild": "Создать ребенка",
        "register.joinChild": "Присоединиться к существующему",
        "register.childName": "Имя ребенка",
        "register.timezone": "Часовой пояс",
        "register.birthdate": "Дата рождения",
        "register.joinCode": "Код присоединения",
        "register.joiningWithCode": "Регистрация для присоединения к ребёнку (код: {{code}})",

        "nav.invite": "Пригласить",
        "invite.title": "Пригласить опекуна",
        "invite.hint": "Отправьте эту ссылку тому, кого хотите пригласить. Он зарегистрируется по ней и получит доступ к ребёнку.",
        "invite.create": "Создать ссылку-приглашение",
        "invite.creating": "Создание...",
        "invite.linkLabel": "Ссылка для регистрации",
        "invite.copy": "Копировать",
        "invite.copied": "Скопировано",
        "invite.code": "Код",
        "invite.expiresAt": "Действует до: {{when}}",
        "invite.errorStatus": "Ошибка {{status}}",
        "invite.networkError": "Ошибка сети",
        "invite.noChildren": "Пока нет детей",
      },
    },
  },
  lng: localStorage.getItem("lang") || "en",
  fallbackLng: "en",
  interpolation: {
    escapeValue: false,
  },
});

export default i18n;
